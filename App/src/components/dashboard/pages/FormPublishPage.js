          ...
          >
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1.5 }}>
              {Array.from({ length: 12 }, (_, i) => i + 1).map((month) => {
                const group = cycleGroups.find(g => g.months.includes(month));
                const themeId = group ? surveyCycleConfig[group.key] : null;
                const theme = surveyTypes.find(t => t.id === themeId);
                ...
                return (
                  <Box
                    ...
                    sx={{
                      ...
                      p: 2,
                      borderRadius: 1.5,
                      bgcolor: isCurrentMonth ? theme?.color : theme?.bgColor,
                      border: isCurrentMonth ? `2px solid ${theme?.color}` : '1px solid #e5e7eb',
                      textAlign: 'center',
                      transition: 'all 0.2s'
                    }}
                  >
                    <Typography
                      sx={{
                        fontSize: '0.75rem',
                        fontWeight: 600,
                        color: isCurrentMonth ? '#fff' : '#64748b',
                        mb: 0.5
                      }}
                    >
                      {month}月
                    ...
                    <Typography
                      sx={{
                        fontSize: '0.7rem',
                        fontWeight: 700,
                        color: isCurrentMonth ? '#fff' : theme?.color
                      }}
                    >
                      {theme?.label}
                    </Typography>

...
function buildInsightPrompt(
  ...
  return `あなたは飲食店の月次レポートのAIアナリストです。${targetYearMonth}の店舗「${storeName}」の今月のデータを分析し、今月この店舗がどのような状況にあるかを明らかにしてください。

【分析の目的】
- 分析の主役は「今月のこの店舗のデータ」です
- 前月データや全体平均は、今月の状況を客観的に理解するための比較材料として使ってください
- 過去の問題点を列挙するのではなく、「今月はこうだった」「今月はこの点が特徴的だった」という今月の分析を行うこと
- ポジティブな特徴も、改善が必要な点も、今月のデータから見えるものをバランスよく取り上げること

【重要: データの性質について】
- このデータは来店客へのアンケート調査の結果であり、「回答数」は来店数ではなくアンケートへの回答数を意味する
- 回答数が少ない場合は個別の回答に左右されやすいため、傾向を断定しすぎないこと
- 一定の回答数がある場合は、件数よりも比率（ポジティブ率・ネガティブ率など）に注目して分析すること
- 「○○が△件」ではなく「回答者の○%が△と評価」のように比率ベースで語ることを優先する
...
【分析の着眼点 — 今月のデータから何が見えるか】
- 今月の顧客コメントに見られる特徴的なパターンや共通テーマ
- 前月と比較して今月に変化が見られる点（改善・悪化の両方）
- 特定セグメントで今月特に顕著な傾向（良い点・気になる点の両方）
- 今月のセグメント別QSC評価と店舗全体の平均との差が大きい項目
- 今月のQSC項目でポジティブ率が特に高い項目やネガティブ率が特に高い項目（20%以上）
- 前月と比べて今月に5%以上変動した項目

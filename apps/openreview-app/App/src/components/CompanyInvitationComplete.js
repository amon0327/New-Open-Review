...
export default function CompanyInvitationComplete() {
  ...
  const handleInvitationComplete = async () => {
    try {
      ...
      console.log('CompanyInvitationComplete - token:', token);

      // 認証情報の取得
      const { data: sessionData } = await supabase.auth.getSession();

      console.log('CompanyInvitationComplete - sessionData:', sessionData);

      if (!sessionData.session) {
        throw new Error('ログインが確認できません。再度ログインしてください。');
      }

      console.log('CompanyInvitationComplete - calling Edge Function with token:', token);

      // Edge Functionを使用して招待を完了
      const { data, error } = await supabase.functions.invoke('complete-company-invitation', {
        body: {
      ...
      console.log('CompanyInvitationComplete - Edge Function response:', { data, error });
      ...
      console.log('CompanyInvitationComplete - 登録成功:', data.company);
      ...
      console.error('CompanyInvitationComplete - エラー:', err);
      ...
      } else if (err.message.includes('既にこの企業のメンバー')) {
        errorMessage = '無効なURLです。既にこの企業のメンバーです。';

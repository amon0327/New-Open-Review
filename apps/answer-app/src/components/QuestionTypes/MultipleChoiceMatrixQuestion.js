...
const MultipleChoiceMatrixQuestion = ({ 
  question, 
  themeColor, 
  currentQuestion, 
  totalQuestions, 
  ...
  useEffect(() => {
    ...
    if (question.existingAnswers && Array.isArray(question.existingAnswers)) {
      setSelectedChoices(question.existingAnswers);
    }
  }, [question.existingAnswers]);

  const handleChoiceChange = (choiceNumber, isChecked) => {
    let newSelectedChoices;
    
    if (isChecked) {
      newSelectedChoices = [...selectedChoices, choiceNumber];
    } else {
      newSelectedChoices = selectedChoices.filter(num => num !== choiceNumber);
    }
    
    setSelectedChoices(newSelectedChoices);
    onAnswerChange(question.id, {
      questionTypeId: 6,
      answers: newSelectedChoices.length > 0 ? newSelectedChoices : null
    });
  };
    ...
    <>
      <Container maxWidth={false} sx={{ width: '100%', px: 0 }}>
        <Box sx={{ pt: '50px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          {/* Multiple Choice Options - 2 Column Matrix */}
          <Box sx={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
            <Box sx={{ 
              ...
              {question.question_option_choices?.sort((a, b) => a.choice_number - b.choice_number).map((choice, index) => {
                const isSelected = selectedChoices.includes(choice.choice_number);
                ...
                return (
                  ...
                  >
                    <Button
                      variant="contained"
                      onClick={() => handleChoiceChange(choice.choice_number, !isSelected)}
                      sx={{

...
serve(async (req) => {
  ...
  try {
    ...
    // 注: status フィルタを撤廃。completed の招待 URL を未ログインで踏んだ
    // ユーザーにも 招待詳細 + LINE ログインボタンを表示し、ログイン後に
    // /complete で alreadyMember 判定 → ホームへ進める動線を用意する。
    // 別人が completed 招待を踏んだケースは /complete の status チェックで
    // 弾かれるので 招待の使い回しは不可。
